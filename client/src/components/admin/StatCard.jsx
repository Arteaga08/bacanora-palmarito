...
const StatCard = ({ title, value, icon, trend, isDanger }) => {
  return (
    <div className="bg-brand-cream border border-brand-dark/10 p-8 flex flex-col justify-between group hover:border-brand-orange/30 transition-colors duration-500">
      <div className="flex justify-between items-start mb-6">
        <span className="text-[10px] uppercase tracking-[0.3em] text-brand-dark/50 font-medium">
          {title}
        </span>
        <div
          className={`${isDanger ? "text-rose-600" : "text-brand-orange"} opacity-80 group-hover:opacity-100 transition-opacity`}
        >
          {icon}
        </div>
      </div>
